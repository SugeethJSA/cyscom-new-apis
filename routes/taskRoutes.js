...
taskRoutes.delete("/:id", requireAuth, async (req, res, next) => {
  try {
    const { id } = req.params;
    
    // Check if task exists
    const checkRes = await query("SELECT * FROM tasks WHERE id = $1", [id]);
    if (checkRes.rows.length === 0) {
      return res.status(404).json({ error: "not_found", message: "Task not found." });
    }

    // Soft delete: set status to 'archived'
    await query("UPDATE tasks SET status = 'archived', updated_at = CURRENT_TIMESTAMP WHERE id = $1", [id]);
    
    res.json({ success: true, message: "Task successfully archived." });
  } catch (error) {
    next(error);
  }
});
